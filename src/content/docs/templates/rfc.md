...
Release shapes (staged rollout, shadow run, champion-challenger, artifact rollback, backfill): see [Writing an RFC](/productos-site/guides/product-specs/#rollout). Pick the shape that fits the change; feature-flag on/off is one option, not the only one.
